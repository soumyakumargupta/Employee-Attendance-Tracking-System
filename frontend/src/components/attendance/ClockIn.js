...
const ClockIn = () => {
  ...
  const handleInitiateClockIn = async () => {
    if (!location) {
    ...
    setLoading(true);
    try {
      await apiService.initiateClockIn({
      ...
      console.error('Clock in initiation error:', error);
      toast.error(error.response?.data?.message || 'Failed to initiate clock in');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyOtp = async (e) => {
    ...
    if (!location) {
      toast.error('Location is required for clock in verification');
      return;
    }

    setOtpLoading(true);
    try {
      await apiService.verifyClockIn({
        otp: otp.trim(),
        latitude: location.latitude,
        longitude: location.longitude
      });
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <form onSubmit={handleVerifyOtp} className="space-y-4">
                    ...
                    <div className="flex space-x-3">
                      ...
                      >
                        {otpLoading ? 'Verifying...' : 'Verify & Clock In'}
                      </button>
                      <button
                        type="button"
